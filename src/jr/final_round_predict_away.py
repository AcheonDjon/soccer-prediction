...
gdf =pd.read_csv('./data/NSL_Knockout_Round_Games2.csv')
...
def average_stats_first_team(team_name):
   filtered_home_df = df[df["AwayTeam"] == team_name]
   filtered_home_df['Away_ToP'] = 1-filtered_home_df['Home_ToP']
   average_home = pd.DataFrame(
      {
        'Home_xG' :  filtered_home_df['Away_xG'].mean(),
        'Home_shots' :  filtered_home_df['Away_shots'].mean(),
        'Home_corner' :  filtered_home_df['Away_corner'].mean(),
        'Home_PK_Goal' :  filtered_home_df['Away_PK_Goal'].mean(),
        'Home_PK_shots' :  filtered_home_df['Away_PK_shots'].mean(),
        'Home_ToP' :  filtered_home_df['Away_ToP'].mean(),
...
def average_stats_second_team(team_name):
    filtered_away_df = df[df["AwayTeam"] == team_name]
    filtered_away_df['Away_ToP'] = 1-filtered_away_df['Home_ToP']
    ...
    average_away = pd.DataFrame(
        {
          ...
          'Away_ToP' :  filtered_away_df['Away_ToP'].mean(),
